module Basic = {
  ...
  let make = (~children) => {
    <div
      className=%twc("text-background my-4 p-1 border bg-gray_light border-gray rounded shadow-sm")>
...
module Presentation = {
  ...
  let renderPresentationCard = (lang, card: t, id: string) => {
    let buttonStyle = %twc(
      "cursor-pointer bg-button_bg py-2 px-4 text-button_fg text-base inline-flex items-center rounded font-semibold font-sans hover:bg-button_bg_hover hover:text-button_fg_hover ease-in duration-100"
    )
