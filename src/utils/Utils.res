...
let render_presentation_card = (card: presentation_card, lang: Lang.lang, id: string) => {
...
module PresentationCards = {
  ...
  let make = (~cards: array<presentation_card>) => {
    ...
    <div className=%tw("flex flex-row flex-wrap items-stretch")>
      {cards
      ->Belt.Array.mapWithIndex((i, card) => render_presentation_card(card, lang, string_of_int(i)))
      ->React.array}
    </div>
  }
}
