...
let catala_exe:
  (string, React.ref(Js.Nullable.t(Dom.element)), string, bool) => string = [%bs.raw
  {|
...
let make = () => {
  ...
  let render_module_select = (modu: catala_module, id: string): React.element =>
    <div key=id>
      <button
        ...
        className=[%tw
          "bg-secondary shadow text-white px-4 py-2 mt-1 uppercase"
        ]>
        {modu.title}
      </button>
    </div>;
  let exec_catala_and_retrive_output =
    ...
    | Some(id) =>
      ReactDOM.render(<div dangerouslySetInnerHTML={"__html": output} />, id)
    | None => ()
    };
  };
  <>
    <div className=[%tw "flex flex-col flex-grow"]>
      ...
      <p>
        ...
        refer to "
          french={js|Les fonctionnalités de ce bac à sable sont limitées
        ...
        <Utils.TextLink target="https://github.com/CatalaLang/catala">
          <Lang.String
            english="the readmes of the GitHub repository"
      ...
      to set up your IDE."
          french={js| pour plus d'information sur comment
      configurer votre EDI.|js}
        />
      </p>
      <p>
        <Lang.String
      ...
      <div className=[%tw "flex flex-col flex-grow mt-8"]>
        ...
        <div
          className=[%tw
            "flex flex-row flex-wrap justify-center p-2 bg-secondary shadow"
          ]>
          <div className=[%tw "pr-4 text-white my-1"]>
            <Lang.String
              english="Scope to execute:"
              french={js|Champ d'application à exécuter :|js}
            />
          </div>
          <input
        ...
        <div
          id="catala-output"
          className=[%tw
            "my-4 overflow-x-auto border-tertiary border-solid border-2 p-2"
          ]>
          <pre>
